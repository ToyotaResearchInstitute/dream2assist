...
class Dreamer(nn.Module):
    ...
    def __call__(self, obs, reset, state=None, reward=None, training=True):
        ...
        # print("action call")
        # print([f" {k}: {v.shape}" for k, v in policy_output.items()])
        # print([f" {k}: {v}" for k, v in policy_output.items()])
    ...
    def _policy(self, obs, state, training):
        ...
        latent, _ = self._wm.dynamics.obs_step(latent, action, embed, obs["is_first"], self._config.collect_dyn_sample)
        ...
        # print(f"action policy {action.shape}")
...
def make_env(config, logger, mode, train_eps, eval_eps):
    ...
    if suite == "dmc":
        ...
        env = dmlab.DeepMindLabyrinth(task, mode if "train" in mode else "test", config.action_repeat)
        env = wrappers.OneHotAction(env)
    elif suite == "racing":
        from shared_control_racing import SharedControlRacingWrapper
        from shared_control_racing.render_utils import Render

        env = SharedControlRacingWrapper(
            task,
            config.action_repeat,
            config.size,
        ...
        renderer = functools.partial(Render, env.env)
        # env = wrappers.OneHotAction(env)
    elif suite == "decision":
        from shared_control_racing import DecisionMakerWrapper
        from shared_control_racing.render_utils import Render

        print(config.human_agent_path)
        env = DecisionMakerWrapper(
            task, config.action_repeat, config.size, human_agent_path=config.human_agent_path, config=config,
        )
        renderer = functools.partial(Render, env.env)
        # env = wrappers.OneHotAction(env)
    elif suite == "humandecision":
        from shared_control_racing import HumanDecisionRacingWrapper
        from shared_control_racing.render_utils import Render

        # Load the external human agent policy.
        directory = config.traindir
        ado_action_space = spaces.Discrete(5)
        ...
        # Load the ado-agent
        agent = Dreamer(ado_observation_space, ado_action_space, config, tmp_logger, dataset=None,).to(config.device)
        agent.requires_grad_(requires_grad=False)
        agent.load_state_dict(torch.load(config.human_agent_path))
        agent._should_pretrain._once = False
        eval_policy = functools.partial(agent, training=False)

        env = HumanDecisionRacingWrapper(
            task,
            config.action_repeat,
            config.size,
            # human_agent_path=config.human_agent_path,
            config=config,
            render_mode=None if mode == "eval" else "no_render",
            track_csv=str(make_absolute_path(config.track_csv)),
    ...
    if (mode == "train") or (mode == "eval"):
        callbacks = [functools.partial(ProcessEpisodeWrap.process_episode, config, logger, mode, train_eps, eval_eps, renderer,)]
...
class ProcessEpisodeWrap:
    ...
    @classmethod
    def process_episode(cls, config, logger, mode, train_eps, eval_eps, renderer, episode, info):
        ...
        if mode == "train":
            ...
            renderer = renderer(info["agent_actions"], info["agent_states"], episode["reward"])
            video = episode["image"]
        ...
        wandb.log({f"{mode}_return": score})
        wandb.log({f"{mode}_length": length})
        wandb.log({f"{mode}_episodes": len(cache) if mode == "train" else episode_num})
...
def main(config):
    ...
    wandb.init(
        ...
        project="shared-decision-making",
    ...
    if not config.offline_traindir:
        ...
        if hasattr(acts, "discrete"):
            ...
            random_actor = torchd.independent.Independent(
                torchd.uniform.Uniform(
                    torch.Tensor(acts.low).repeat(config.envs, 1), torch.Tensor(acts.high).repeat(config.envs, 1),
                ),
                1,
            )
        ...
        def random_agent(o, d, s, r):
            ...
            # print(f"action random {action.shape}")
    ...
    agent = Dreamer(train_envs[0].observation_space, train_envs[0].action_space, config, logger, train_dataset,).to(
        config.device
    )
    agent.requires_grad_(requires_grad=False)
    ...
    while agent._step < config.steps:
        ...
        if config.video_pred_log:
            ...
            # pass array of dim (batch, time, channel, width, height)
            wandb.log({"eval_openl": wandb.Video(np.moveaxis(to_np(video_pred), -1, -3))})
        print("Start training.")
        state = tools.simulate(agent, train_envs, config.eval_every, state=state)
        torch.save(agent.state_dict(), logdir / "latest_model.pt")
        torch.save(agent.state_dict(), logdir / f"model_checkpoint_{agent._step:06d}.pt")
