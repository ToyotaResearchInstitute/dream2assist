...
class Plan2Explore(nn.Module):
    def __init__(self, config, world_model, reward=None):
        ...
        self._networks = nn.ModuleList([networks.MLP(**kw) for _ in range(config.disag_models)])
        kw = dict(wd=config.weight_decay, opt=config.opt, use_amp=self._use_amp)
        self._model_opt = tools.Optimizer(
            "explorer", self.parameters(), config.model_lr, config.opt_eps, config.grad_clip, **kw
        )
